...
const Section = ({ title, children }) => (
  <div style={{ marginBottom: '2.25rem' }}>
    <h2 style={{ fontSize: '1.2rem', fontWeight: 700, color: 'var(--color-secondary-100)', marginBottom: '0.85rem', paddingBottom: '0.5rem', borderBottom: '1px solid var(--color-primary-25)' }}>{title}</h2>
    <div style={{ fontSize: '0.95rem', color: 'var(--color-charcoal)', lineHeight: 1.75 }}>{children}</div>
  </div>
);
const P = ({ children }) => <p style={{ margin: '0 0 0.75rem' }}>{children}</p>;
const Ul = ({ items }) => (
  <ul style={{ paddingLeft: '1.5rem', margin: '0 0 0.85rem' }}>
  ...
  return (
    <div>
      ...
      <PageHero
        ...
        subtitle="What data we collect, why, and how you control it. We do not sell your data. We do not run ads targeting personal data. Ever."
      />

      <section style={{ background:'#fff', padding:'4rem 0' }}>
        <div className="u-content-width">
          <div style={{ maxWidth:'48rem', margin:'0 auto' }}>
            <div style={{ background:'var(--color-primary-5)', border:'1px solid var(--color-primary-25)', borderRadius:8, padding:'1.25rem 1.5rem', marginBottom:'2.5rem' }}>
              <p style={{ fontSize:'0.95rem', color:'var(--color-secondary-100)', margin:0, lineHeight:1.65 }}>
                StackRank365 is an independent platform for Microsoft professionals. This policy explains exactly what data we collect, why, and how you can control it.
              </p>
            </div>
            ...
            <div style={{ borderTop:'1px solid var(--color-primary-25)', paddingTop:'1.5rem', marginTop:'2rem', display:'flex', gap:'1.75rem', flexWrap:'wrap' }}>
              <a href="#" onClick={(e)=>{ e.preventDefault(); onNavigate?.('landing'); }} style={{ fontSize:'0.95rem', color:'var(--color-primary-100)', textDecoration:'none', fontWeight:600 }}>← Back to home</a>
              <a href="mailto:privacy@stackrank365.com" style={{ fontSize:'0.95rem', color:'var(--color-primary-100)', textDecoration:'none', fontWeight:600 }}>Contact privacy team</a>
            </div>
          </div>
        </div>
      </section>
