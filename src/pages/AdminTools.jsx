...
async function safeJson(res) {
  ...
  if (text.trim().startsWith('<')) throw new Error('Got HTML instead of JSON — check SB URL/key');
...
const SUITES = [
  { id: 'landing', name: '🌐 Landing Page', tests: [
    { id:'TC-001', name:'Page title correct', priority:'P1', run: async () => {
      ...
      return h.includes('StackRank365') ? pass('Title correct ✓') : fail('StackRank365 not found in page');
    }},
    { id:'TC-002', name:'Nav links in DOM', priority:'P1', run: async () => {
      ...
      return missing.length ? fail('Missing in DOM: '+missing.join(', ')) : pass('All nav links in live DOM ✓');
    }},
    { id:'TC-003', name:'Load time < 5s', priority:'P2', run: async () => {
      ...
      return ms > 5000 ? fail(ms+'ms exceeds 5s') : pass('Loaded in '+ms+'ms ✓');
    }},
    { id:'TC-004', name:'Privacy page loads', priority:'P1', run: async () => {
      ...
      return h.includes('StackRank365') ? pass('Privacy page shell loads ✓') : fail('Shell missing');
    }},
    { id:'TC-005', name:'Pricing page loads', priority:'P2', run: async () => {
      ...
      return h.includes('StackRank365') ? pass('Pricing page loads ✓') : fail('Shell missing');
    }},
  ]},
  { id: 'auth', name: '🔐 Authentication', tests: [
    { id:'TC-020', name:'Sign up new email', priority:'P1', run: async () => {
      const e = 'auto_'+Date.now()+'@example.com';
      const r = await sbAuth('/auth/v1/signup', { email: e, password: 'TestPass123!' });
      const d = await safeJson(r);
      return (d.user?.id || d.id) ? pass('Signup accepted: '+e+' ✓') : warn('Returned: '+(d.error_description||d.msg||JSON.stringify(d).slice(0,80)));
    }},
    { id:'TC-024', name:'Sign in valid credentials', priority:'P1', run: async () => {
      ...
      const d = await safeJson(r);
      return d.access_token ? pass('Signed in as '+d.user?.email+' ✓') : fail('Sign in failed: '+(d.error_description||'No token returned'));
    }},
    { id:'TC-025', name:'Wrong password rejected', priority:'P1', run: async () => {
      ...
      return !r.ok ? pass('Wrong password rejected ('+r.status+') ✓') : fail('SECURITY: wrong password returned 200!');
    }},
    { id:'TC-026', name:'Fake email rejected', priority:'P1', run: async () => {
      ...
      return !r.ok ? pass('Fake email rejected ('+r.status+') ✓') : fail('SECURITY: fake email returned 200!');
    }},
    { id:'TC-027', name:'Auth rejects missing token', priority:'P1', run: async () => {
      const r = await fetch(SB+'/auth/v1/user', { headers: { apikey: KEY } });
      return (r.status===401||r.status===403) ? pass('Unauthenticated request rejected ('+r.status+') ✓') : fail('Expected 401/403, got '+r.status);
    }},
  ]},
  { id: 'database', name: '🗄️ Database & Schema', tests: [
    { id:'TC-DB-01', name:'profiles table accessible', priority:'P1', run: async () => {
      ...
      const d = await safeJson(r);
      return pass('profiles OK ('+d.length+' rows via RLS) ✓');
    }},
    { id:'TC-DB-02', name:'cert_catalog ≥ 250 rows', priority:'P1', run: async () => {
      ...
      return total < 250 ? fail('Only '+total+' rows — run sync-catalog function') : pass(total+' rows in cert_catalog ✓');
    }},
    ...
    { id:'TC-DB-07', name:'profiles has all Phase 3+4 columns', priority:'P1', run: async () => {
      ...
      return pass('All new columns present ✓');
    }},
  ]},
  { id: 'scoring', name: '📊 Scoring Logic', tests: [
    { id:'TC-SC-01', name:'Verified cert → score_multiplier=1.00', priority:'P1', run: async () => {
      ...
      if (!rows.length) return skip('No verified certs yet — verify one first');
      const bad = rows.filter(r => parseFloat(r.score_multiplier) !== 1.00);
      return bad.length ? fail(bad.length+' verified certs have wrong multiplier') : pass(rows.length+' verified → 1.00 ✓');
    }},
    { id:'TC-SC-02', name:'Unverified cert → score_multiplier=0.25', priority:'P1', run: async () => {
      ...
      return bad.length ? fail(bad.length+' wrong') : pass(rows.length+' unverified → 0.25 ✓');
    }},
    { id:'TC-SC-03', name:'cert_catalog schema complete', priority:'P1', run: async () => {
      ...
      if (!r.ok) return fail('HTTP '+r.status);
      const d = await safeJson(r);
      return pass('Schema OK. Sample: '+(d[0]?.certification_name||'(empty)').slice(0,50)+' ✓');
    }},
  ]},
  { id: 'security', name: '🔒 Security', tests: [
    { id:'TC-170', name:'RLS — private fields not exposed to anon', priority:'P1', run: async () => {
      const r = await sbRest('/rest/v1/profiles?select=email,fraud_score&limit=5');
      if (!r.ok) return pass('Anon query blocked by RLS ✓');
      const rows = await safeJson(r);
      return rows.some(rw=>rw.email) ? fail('SECURITY: email field exposed via RLS!') : pass('Private fields not in anon response ✓');
    }},
    { id:'TC-172', name:'service_role key not in JS bundle', priority:'P1', run: async () => {
      ...
      return js.includes('service_role') ? fail('SECURITY: service_role in client JS!') : pass('service_role not found in bundle ✓');
    }},
    { id:'TC-173', name:'Page served over HTTPS', priority:'P1', run: async () =>
      location.protocol==='https:' ? pass('HTTPS confirmed ✓') : fail('Not on HTTPS!')
    },
    { id:'TC-174', name:'REST API returns JSON content-type', priority:'P1', run: async () => {
      ...
      return ct.includes('application/json') ? pass('API returns application/json ✓') : fail('API returned: '+ct+' (HTTP '+r.status+')');
    }},
    { id:'TC-175', name:'Auth rejects unauthenticated user requests', priority:'P1', run: async () => {
      const r = await fetch(SB+'/auth/v1/user', { headers: { apikey: KEY } });
      return (r.status===401||r.status===403) ? pass('Unauthenticated rejected ('+r.status+') ✓') : fail('Expected 401/403, got '+r.status);
    }},
  ]},
  { id: 'edge_fns', name: '⚡ Edge Functions', tests: [
    { id:'TC-EF-01', name:'verify-cert: missing params → 400', priority:'P1', run: async () => {
      const r = await sbFn('verify-cert', {});
      return r.status===400 ? pass('Returns 400 for missing params ✓') : fail('Expected 400, got '+r.status);
    }},
    { id:'TC-EF-02', name:'analyse-resume: missing pdf → 400', priority:'P1', run: async () => {
      const r = await sbFn('analyse-resume', {});
      return r.status===400 ? pass('Returns 400 ✓') : fail('Expected 400, got '+r.status);
    }},
    { id:'TC-EF-03', name:'recruiter-match: missing JD → 400', priority:'P1', run: async () => {
    ...
    { id:'TC-EF-04', name:'detect-fake-profiles: deployed & runs', priority:'P2', run: async () => {
      ...
      const d = await safeJson(r);
      return (r.ok && d.checked!==undefined) ? pass('deployed: checked='+d.checked+', flagged='+d.flagged+' ✓') : fail('HTTP '+r.status+': '+(d.error||JSON.stringify(d).slice(0,60)));
    }},
    { id:'TC-EF-05', name:'batch-verify-certs: deployed', priority:'P2', run: async () => {
      const r = await sbFn('batch-verify-certs', { limit: 5 });
      const d = await safeJson(r);
      return r.ok ? pass('deployed: processed='+d.processed+', verified='+d.verified+' ✓') : fail('HTTP '+r.status+': '+(d.error||JSON.stringify(d).slice(0,60)));
    }},
  ]},
  { id: 'data', name: '🗂️ Data Integrity', tests: [
    { id:'TC-DI-01', name:'cert_catalog: no null certification_uid', priority:'P1', run: async () => {
      ...
      if (!r.ok) return fail('HTTP '+r.status);
      const rows = await safeJson(r);
      return rows.length ? fail(rows.length+' entries have null certification_uid!') : pass('All entries have non-null UID ✓');
    }},
    { id:'TC-DI-02', name:'certifications: verification_status valid', priority:'P1', run: async () => {
      ...
      const rows = await safeJson(r);
      return rows.length ? fail(rows.length+' certs have invalid status') : pass('All verification_status values valid ✓');
    }},
    { id:'TC-DI-03', name:'profiles: tier values valid', priority:'P1', run: async () => {
      ...
      if (!r.ok) return skip('RLS blocks anon check');
      const rows = await safeJson(r);
      return rows.length ? fail(rows.length+' profiles have invalid tier') : pass('All tier values valid ✓');
    }},
  ]},
];
      ...
      <div style={{background:'#1e3a5f',padding:'1rem 1.5rem',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:'0.75rem'}}>
        <div>
          ...
          <p style={{color:'#93c5fd',fontSize:12,margin:'3px 0 0'}}>
            {running ? 'Running — '+done+' / '+ALL_TESTS.length :
             done ? 'Complete — ✅ '+stats.pass+'  ❌ '+stats.fail+'  ⏭ '+stats.skip :
             ALL_TESTS.length+' tests · '+SUITES.length+' suites'}
          </p>
        </div>
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          ...
          {running
            ? <button onClick={()=>{stopRef.current=true;}} style={{padding:'7px 14px',background:'#ef4444',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:600,cursor:'pointer'}}>■ Stop</button>
            : <button onClick={runAll} style={{padding:'7px 14px',background:'#22c55e',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:600,cursor:'pointer'}}>▶ Run All Tests</button>
          }
          <button onClick={()=>{sessionStorage.removeItem(SESSION_KEY);window.location.reload();}}
            style={{padding:'7px 14px',background:'rgba(255,255,255,.1)',color:'#fff',border:'none',borderRadius:8,fontSize:12,cursor:'pointer'}}>🔒 Lock</button>
        </div>
      </div>
